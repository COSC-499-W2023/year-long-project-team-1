import { AttributeType, CognitoIdentityProviderClient, ListUsersCommand, ListUsersInGroupCommand, UserType } from "@aws-sdk/client-cognito-identity-provider";
...
export async function getUsrInGroupList(role: string){
    const res = await client.send(new ListUsersInGroupCommand({
        UserPoolId: userPoolId,
        GroupName: role,
    }));
    if(res.Users){
...
function parseUsersInfo(users: UserType[]){
    ...
    users?.forEach(user => {
        const parsedAttributes = user.Attributes? parseAttributes(user.Attributes) : {};
        result.push({
            ...parsedAttributes,
